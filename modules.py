...
m_pins = {"t1_e":10, "t2_e":11, "t2_f":12}						# t1_empty, t2_empty, t2_full
ops = {"on": True, "off": False, "up": "increase", "down": "decrease", "switchoff": False, "turnoff": False, "turnon": True, "switchon": True}
low, high = 0, 1
cmd = ["Nill", "Nill"]
...
def water():
	...
	while events["water"].isset():
		if ((not get(m_pins["t2_e"])) and get(m_pins["t1_e"])) or get(dev_list["motor"]):
			if not get(dev_list["motor"]):
				toggle(dev_list["motor"], True)			#turn on
			while get(m_pins["t1_e"]):
				if get(m_pins["t2_f"]):
					break
...
def play(dir_name):
	...
	for root, dirs, files in os.walk(music_path):
		found = filter(lambda x: (check_name.match(x) != None), dirs)
		if found:
